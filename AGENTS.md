...
- `@dsplay/template-utils` exposes `config`/`media`/`isVertical`/`tval`/`tbval`, read directly (not via hooks) in `src/components/app/index.jsx` and `src/components/post/index.jsx` / `src/components/info/index.jsx`.
